                ...
                <template v-if="users.length">
                    ...
                    <tr v-for="user of users">
                        ...
                        <td style="white-space: nowrap;">
                            <a v-if="user.cf.id" :href="'https://codeforces.com/profile/' + user.cf.id"
                               :style="{ color: user.cf.color }">
                                {{ user.cf.id }} ({{ user.cf.rating }})
                            </a>
                        </td>
                        <td style="white-space: nowrap;">
                            <a v-if="user.nc.uid" :href="'https://ac.nowcoder.com/acm/contest/profile/' + user.nc.uid"
                               :style="{ color: user.nc.color }">
                                {{ user.nc.id }} ({{ user.nc.rating }})
                            </a>
                            <div v-else-if="user.nc.id">{{ user.nc.id }}</div>
                        </td>
                        <td style="white-space: nowrap;">
                            <a :href="'https://atcoder.jp/users/' + user.atc.id">{{ user.atc.id }}</a>
                        </td>
                    </tr>
                </template>
            </template>
